...
def sell(web3_provider, market_maker_address, return_amount, index, maximum_shares):
    fixed_product_market_maker_address_abi = load_evm_abi(
    ...
    contract = web3_provider.eth.contract(
        address=market_maker_address, abi=fixed_product_market_maker_address_abi
    )
    trx_hash = contract.functions.sell(
        fixed_return_amount, index, fixed_maximum_shares
    ).transact()
...
def sell_shares(web3_provider, slug, outcome, num_shares, slippage):
    ...
    pool_balances = get_pool_balances(
        web3_provider, market_maker_address, condition_id, num_outcomes
    )
    sell_amount_in_usdc = calc_sell_amount_in_collateral(
        fixed_share_count, outcome_index, pool_balances, fee
    )
    ...
    trx_hash = contract.functions.sell(
        fixed_return_amount, outcome_index, slippage_fixed_share_count
    ).transact()
    web3_provider.eth.wait_for_transaction_receipt(trx_hash)
