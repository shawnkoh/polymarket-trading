...
def conditional_token_approve_for_all(w3, fixed_product_market_maker_address, status):
    ...
    instance = w3.eth.contract(
        address=conditional_token_address, abi=conditional_token_abi
    )
    receipt = w3.eth.wait_for_transaction_receipt(
        instance.functions.setApprovalForAll(
            fixed_product_market_maker_address, status
        ).transact()
    )
...
def initialize_identity(gas_price=None):
    ...
    if not vigil_key and not rpc_uri:
        raise RuntimeError(
            "Must include either RPC_URI or MATIC_VIGIL_RPC_KEY environment variable"
        )
...
def get_pool_balances(web3_provider, mkt_id, condition_id, num_outcomes):
    ...
    for idx in range(num_outcomes):
        val = contract.functions.getCollectionId(
            parent_collection_id, condition_id, 0x1 << idx
        ).call()
